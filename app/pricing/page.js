  ...
  return (
    <>
      ...
      <main style={{ background: 'var(--bg)', minHeight: 'calc(100vh - 56px)' }}>
        {/* Header */}
        <div style={{ background: '#0D1B3E', padding: '56px 28px 48px', textAlign: 'center' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 28px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, alignItems: 'start' }}>
            {plans.map(plan => (
              <div key={plan.name} style={{
                ...
                border: plan.highlight ? '2px solid #1565C0' : '1px solid #E2E8F0',
                borderRadius: 16,
                overflow: 'hidden',
                position: 'relative',
                ...
                {plan.badge && (
                  <div style={{ background: '#1565C0', color: '#ffffff', fontSize: 11, fontWeight: 700, padding: '5px 0', textAlign: 'center', letterSpacing: '0.04em' }}>
                    {plan.badge}
                  </div>
                )}

                <div style={{ padding: '24px 24px 20px' }}>
                  <h2 style={{ fontSize: 20, fontWeight: 700, color: '#0F172A', marginBottom: 6 }}>{plan.name}</h2>
                  <p style={{ fontSize: 13.5, color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: 20, minHeight: 52 }}>{plan.desc}</p>

                  {/* Price */}
                  {plan.price ? (
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 32, fontWeight: 700, color: '#0F172A', letterSpacing: '-0.02em' }}>{plan.price}</div>
                      {plan.priceNote && <p style={{ fontSize: 12.5, color: 'var(--text-muted)', marginTop: 4 }}>{plan.priceNote}</p>}
                    </div>
                  ) : (
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 24, fontWeight: 700, color: '#0F172A' }}>Free to start</div>
                  ...
                  <Link href={plan.ctaHref} style={{
                    ...
                    background: plan.highlight ? '#1565C0' : '#0D1B3E',
                    color: '#ffffff',
          ...
          {/* Bottom CTA */}
          <div style={{ marginTop: 56, background: '#0D1B3E', borderRadius: 16, padding: '40px', textAlign: 'center' }}>
